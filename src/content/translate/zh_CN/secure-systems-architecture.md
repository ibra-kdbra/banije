 ...
 * **网络工程师的观点：** 这是 MAC 地址、交换机和局域网 (LAN) 的领域。 主要协议是以太网和**ARP（地址解析协议）**，它将 IP 地址（第 3 层）映射到 MAC 地址（第 2 层） [^4]。 该层负责将帧发送到“同一”本地网段上的正确设备。 
 ...
 * **网络工程师的观点：** 这是IP 地址和路由层。 它是关于在不同网络之间移动数据包。 路由器在这一层运行，根据目标 IP 地址做出决策，将数据包转发到最终目的地。 这里存在诸如**ICMP**（用于 ping 和跟踪路由）和 IGMP 之类的协议 [^9]。 
* **黑客的观点：** 第 3 层攻击的重点是破坏路由和欺骗身份。 
* **IP 欺骗：** 攻击者使用伪造的源 IP 地址创建 IP 数据包。 这是**拒绝服务 (DoS)** 攻击中使用的主要技术。 在 **Smurf 攻击**中，攻击者向网络的广播地址发送大量 ICMP 回显请求 (ping)，将源 IP 欺骗为受害者的 IP。 然后网络上的所有主机都会回复受害者，使其不堪重负 [^10]。 
...
* **黑客的观点：** WAF 规避是一项完善的规则。 攻击者使用工具来探测 WAF、识别供应商和规则集，并制作语法有效但不会触发 WAF 签名的有效负载 [^24]。 
...
* **黑客的观点：** 规避技术包括对数据包进行分段、使用加密（入侵检测系统无法检查加密流量，除非执行 SSL/TLS 解密，这在计算上是昂贵的）以及修改攻击负载以避免匹配已知签名。 
...
* **（拒绝服务）：** 攻击者能否用大量请求淹没此端点以淹没服务 or 数据库？ （防御：速率限制）。 
 ...
 * **服务器端请求伪造 (SSRF)：** 攻击者可以强制服务器端应用程序向任意域发出 HTTP 请求的漏洞。 在云环境中，这可用于访问云提供商的元数据服务，这可能会泄露临时安全凭证 [^32]。 
* **开发人员的观点：** 当应用程序获取用户提供的 URL 并在未经适当验证的情况下从中获取内容时，就会出现 SSRF 漏洞。 防御措施是维护一个严格的允许应用程序请求的域和协议的允许列表。 
* **不安全的反序列化：** 当应用程序在未经适当验证的情况下反序列化不受信任的用户提供的数据时，就会出现此漏洞。 攻击者可以制作恶意序列化对象，该对象在反序列化时可能导致远程代码执行 [^33]。 

#### **8.2 人的因素：社会工程**

 * **黑客的观点：** 人类往往是最薄弱的环节。 社会工程是操纵人们采取行动或泄露机密信息的艺术。 
* **网络钓鱼：** 发送看似来自合法来源的欺诈性电子邮件，以诱骗受害者泄露敏感信息或部署恶意软件。 **鱼叉式网络钓鱼**是一种针对特定个人或组织的高度针对性的网络钓鱼形式 [^34]。 
...
* **黑客的观点：** 在 Windows Active Directory 环境中，这是一个定义明确的过程。 攻击者将从第一台机器的内存中转储凭证（使用像 **Mimikatz** 这样的工具 [^36]）），寻找域管理员帐户。 他们可能会使用诸如**传递哈希**之类的技术，他们可以使用用户的密码哈希向其他计算机进行身份验证，而无需明文密码。 
 ...
 * **身份验证（你是谁？）：**
 * **多重身份验证 (MFA)：** 用于保护帐户的最有效的控制措施。 它需要来自不同类别的两个或多个验证因素：您知道的东西（密码）、您拥有的东西（电话）或您的身份（生物识别） [^40]。 
* **授权（你可以做什么？）：**
 * **开发人员的观点：** 这是许多严重错误发生的地方。 一个常见的缺陷称为**不安全的直接对象引用 (IDOR)**。 当应用程序使用用户提供的标识符来访问资源而不执行授权检查时，就会发生这种情况 [^41]。 修复方法是始终验证当前经过身份验证的用户是否有权访问所请求的资源。 

#### **11.2 开发者密码学：基本规则**

 * **规则 1：永远不要使用自己的加密货币。** 密码学的正确性极其困难。 始终使用经过严格审查的标准库（例如 Google 的 Tink, Libsodium） [^42]。 
* **规则 2：使用强大的标准算法。** 对于哈希密码，请使用现代的慢速算法，例如 **Argon2** [^43]。 对于对称加密，请使用 **AES-256-GCM**。 对于非对称加密，请使用 **RSA-4096** 或椭圆曲线加密。 
* **规则 3：密钥管理就是一切。** 密码系统的安全性完全取决于密钥的保密性。 使用专用密钥管理系统 (KMS) 或硬件安全模块 (HSM) 来存储和管理加密密钥 [^44]。 

#### **11.3 供应链安全：新领域**

 * **软件工程师的观点：** 现代应用程序是由数百个开源依赖项组装而成的。 仅这些依赖项之一中的漏洞就会成为您的应用程序中的漏洞。 这是供应链攻击。 
* **Log4Shell（示例）：** Log4j 漏洞是一个灾难性的示例。 一个普遍存在的日志库存在严重的远程代码执行漏洞，使数百万个应用程序立即受到攻击 [^45]。 
* **防御：**
...
* **安全基础镜像：** 从最小的、可信的基础镜像开始（例如，`distroless`或者`alpine`) 减少攻击面 [^47]. 
...
* **秘密管理：** 不要将秘密以纯文本形式存储在 ConfigMap 中。 使用内置的 Kubernetes Secrets 对象，但为了获得更高的安全性，请与 HashiCorp Vault 等外部机密管理器集成 [^49]. 
...
* **IaC 静态分析：** 在 CI/CD 管道中使用 **Checkov** 或 **tfsec** 等工具来扫描 Terraform 代码中的常见安全问题，例如创建可公开访问的 S3 存储桶或允许从整个互联网进行 SSH 的安全组（`0.0.0.0/0`） [^51]. 
...
[^1]: [Cloudflare - 什么是 OSI 模型？](https://www.cloudflare.com/learning/ddos/glossary/open-systems-interconnection-model-osi/)
[^2]: [Krebs, B. (2012) - 来自微小、无声网络窃听器的日益增长的威胁](https://krebsonsecurity.com/2012/03/the-growing-threat-from-tiny-silent-network-taps/)
[^3]: [Cisco - 什么是 802.1X？](https://www.cisco.com/c/en/us/products/security/what-is-802-1x.html)
[^4]: [Microsoft (2021) - 地址解析协议](https://learn.microsoft.com/en-us/windows-server/administration/performance-tuning/network-subsystem/address-resolution-protocol)
[^5]: [OWASP - 地址解析协议欺骗](https://owasp.org/www-community/attacks/ARP_Spoofing)
[^6]: [Imperva - MAC 洪水](https://www.imperva.com/learn/application-security/mac-flooding/)
[^7]: [Cisco - VLAN 跳跃攻击](https://www.cisco.com/c/en/us/td/docs/switches/lan/catalyst4500/12-2/15-02SG/configuration/guide/config/dhcp.html#wp1102555)
[^8]: [GeeksforGeeks (2023) - 计算机网络中的端口安全](https://www.geeksforgeeks.org/port-security-in-computer-networks/)
[^9]: [Cloudflare - 什么是互联网协议？](https://www.cloudflare.com/learning/network-layer/internet-protocol/)
[^10]: [Cloudflare - Smurf DDoS 攻击](https://www.cloudflare.com/learning/ddos/smurf-ddos-attack/)
[^11]: [Cloudflare - 什么是 BGP 劫持？](https://www.cloudflare.com/learning/security/glossary/bgp-hijacking/)
[^12]: [IETF (2000) - RFC 2827：网络入口过滤：击败采用 IP 源地址欺骗的拒绝服务攻击](https://datatracker.ietf.org/doc/html/rfc2827)
[^13]: [IETF (1981) - RFC 793：传输控制协议](https://datatracker.ietf.org/doc/html/rfc793)
[^14]: [Cloudflare - SYN 洪水攻击](https://www.cloudflare.com/learning/ddos/syn-flood-ddos-attack/)
[^15]: [Nmap - 官方 Nmap 项目网站](https://nmap.org/)
[^16]: [Wikipedia - SYN cookie](https://en.wikipedia.org/wiki/SYN_cookies)
[^17]: [SANS 研究所 (2016) - 实施网络分段](https://www.sans.org/white-papers/37232/)
[^18]: [IETF (2003) - RFC 3069：用于高效地址分配的 VLAN 聚合](https://datatracker.ietf.org/doc/html/rfc3069)
[^19]: [Palo Alto Networks - 什么是 DMZ？](https://www.paloaltonetworks.com/cyberpedia/what-is-a-dmz)
[^20]: [NIST (2020) - 零信任架构](https://csrc.nist.gov/publications/detail/sp/800-207/final)
[^21]: [Kubernetes - 网络政策](https://kubernetes.io/docs/concepts/services-networking/network-policies/)
[^22]: [Palo Alto Networks - 什么是下一代防火墙 (NGFW)？](https://www.paloaltonetworks.com/cyberpedia/what-is-a-next-generation-firewall-ngfw)
[^23]: [OWASP - OWASP 前 10 名](https://owasp.org/www-project-top-ten/)
[^24]: [OWASP - WAF 规避技术](https://owasp.org/www-community/attacks/WAF_Evasion_Techniques)
[^25]: [SANS 研究所 (2001) - 了解入侵检测系统](https://www.sans.org/white-papers/27/)
[^26]: [NSA (2021) - 纵深防御](https://www.nsa.gov/portals/75/documents/what-we-do/cybersecurity/professional-resources/csg-defense-in-depth-20210225.pdf)
[^27]: [OWASP - 威胁建模](https://owasp.org/www-community/Threat_Modeling)
[^28]: [Microsoft (2022) - STRIDE 威胁模型](https://learn.microsoft.com/en-us/azure/security/develop/threat-modeling-tool-threats)
[^29]: [Splunk - 什么是 SIEM？](https://www.splunk.com/en_us/data-insider/what-is-siem.html)
[^30]: [NIST (2012) - SP 800-61 Rev. 2：计算机安全事件处理指南](https://csrc.nist.gov/publications/detail/sp/800-61/rev-2/final)
[^31]: [Lockheed Martin - 网络杀伤链](https://www.lockheedmartin.com/en-us/capabilities/cyber/cyber-kill-chain.html)
[^32]: [OWASP - 服务器端请求伪造](https://owasp.org/www-community/attacks/Server_Side_Request_Forgery)
[^33]: [OWASP - A08:2021 – 软件和数据完整性故障（与不安全反序列化相关）](https://owasp.org/Top10/A08_2021-Software_and_Data_Integrity_Failures/)
[^34]: [CISA - 避免社会工程和网络钓鱼攻击](https://www.cisa.gov/uscert/ncas/tips/ST04-014)
[^35]: [MITRE - ATT&CK 框架](https://attack.mitre.org/)
[^36]: [Depy, B. - mimikatz](https://github.com/gentilkiwi/mimikatz)
[^37]: [Microsoft (2022) - 靠土地为生](https://www.microsoft.com/en-us/security/blog/2022/05/26/living-off-the-land-a-technical-and-strategic-overview-of-lolbins/)
[^38]: [OWASP - 左移](https://owasp.org/www-community/Shift_Left)
[^39]: [OWASP - 静态应用程序安全测试（SAST）](https://owasp.org/www-community/Static_Application_Security_Testing_(SAST))
[^40]: [NIST (2017) - SP 800-63B：数字身份指南：身份验证和生命周期管理](https://pages.nist.gov/800-63-3/sp800-63b.html)
[^41]: [OWASP - A01:2021 – 访问控制损坏（与 IDOR 相关）](https://owasp.org/Top10/A01_2021-Broken_Access_Control/)
[^42]: [Google - Tink 密码库](https://developers.google.com/tink)
[^43]: [The Argon2 Password Hashing Function - Argon2 官方网站](https://www.password-hashing.net/)
[^44]: [AWS - 什么是密钥管理服务？](https://aws.amazon.com/kms/what-is-kms/)
[^45]: [CISA - Apache Log4j 漏洞指南](https://www.cisa.gov/uscert/apache-log4j-vulnerability-guidance)
[^46]: [NTIA - 软件物料清单 (SBOM)](https://www.ntia.gov/SBOM)
[^47]: [GoogleCloudPlatform - 无发行版 Docker 镜像](https://github.com/GoogleCloudPlatform/distroless)
[^48]: [Kubernetes - 使用 RBAC 授权](https://kubernetes.io/docs/reference/access-authn-authz/rbac/)
[^49]: [HashiCorp - 金库](https://www.vaultproject.io/)
[^50]: [Kubernetes - Pod 安全标准](https://kubernetes.io/docs/concepts/security/pod-security-standards/)
